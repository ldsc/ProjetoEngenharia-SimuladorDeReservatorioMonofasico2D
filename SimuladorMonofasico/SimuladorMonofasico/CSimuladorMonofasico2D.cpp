...
void CSimuladorMonofasico2D::run() {
...
std::vector<double> CSimuladorMonofasico2D::calc_H(CProps* props_n, CProps* props_nu, double dt) {
...
std::vector<double> CSimuladorMonofasico2D::calc_Q( double time) {
	std::vector<double> Q(grid->Ntotal() + 1, 0.0);
	Q[0] = well->Qsc(time) * (grid->DTheta() / (2 * PI));
	return Q;
}

std::vector<double> CSimuladorMonofasico2D::calc_X(double pw, std::vector<double> p) {
...
std::vector<std::vector<double>> CSimuladorMonofasico2D::calc_T(CProps* props_n) {
...
std::vector<std::vector<double>> CSimuladorMonofasico2D::calc_eta( CProps* props_nu, double dt) {
	int nt = grid->Ntotal() + 1;
	int nr = grid->Nr();
	int nz = grid->Nz();
...
std::vector<std::vector<double>> CSimuladorMonofasico2D::calc_tau( CProps* props_nu, std::vector<double> p_nu, double pw_nu) {
	int nt = grid->Ntotal();
	int nr = grid->Nr();
	int nz = grid->Nz();
...
void CSimuladorMonofasico2D::plot(std::vector<double> time, std::vector<double> Pw) {
...
void CSimuladorMonofasico2D::plotSurface() {
...
bool CSimuladorMonofasico2D::isErrorNotAcceptable(double dt, std::vector<double> R, CProps* props_nu, double q) {
...
void CSimuladorMonofasico2D::read_data_and_start_objects(std::string nameFile) {
